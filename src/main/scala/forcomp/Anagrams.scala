...
object Anagrams {
  ...
  /**
    * List(('a', 1), ('e', 1), ('t', 1)) -> Seq("ate", "eat", "tea")
    * の場合
    * List(('a', 1), ('e', 1), ('t', 1)) -> Seq("ate")
    * その後に同じ文字かつ同じ回数の場合、同じキーになるので
    * List(('a', 1), ('e', 1), ('t', 1)) -> Seq("ate", "eat")
    * と一致するものがあると、どんどん増えていく（昇順でソートしているので、必ず一致する）
    */
  ...
  /**
    * wordOccurrences(word)で検索対象のキーを取得し、それに一致するものをMapからゲットする
    *
    * @param word
    * @return
    */
